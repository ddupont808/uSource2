...
#pragma warning restore CA1051 // Do not declare visible instance fields
        ...
        public static int GetDXGIStride(DXGI_FORMAT format)
        {
            switch(format)
            {
                ...
                case DXGI_FORMAT.R16G16_SINT:
                    return 4;
                default:
                    throw new NotImplementedException($"Unsupported DXGI_FORMAT.{format}");
            }
        }

        public static byte[] ReadVertexAttributeRaw(int offset, OnDiskBufferData vertexBuffer, RenderInputLayoutField attribute)
        {
            offset = (int)(offset * vertexBuffer.ElementSizeInBytes) + (int)attribute.Offset;

            var stride = GetDXGIStride(attribute.Format);
