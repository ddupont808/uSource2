...
#pragma warning restore CA1001 // Types that own disposable fields should be disposable
    {
        ...
        private void ReadVersion3(BinaryReader reader, BinaryWriter outWrite, BinaryReader outRead)
        {
            ...
            if (compressionMethod == 0)
            {
                ...
                var output = new byte[compressedSize];
            ...
            else if (compressionMethod == 1)
            {
                ...
                outWrite.Write(output, 0, output.Length);
            ...
            else if (compressionMethod == 2)
            {
                ...
                var input = reader.ReadBytes(compressedSize);
                var zstd = new ZstdNet.Decompressor();
            ...
            try
            {
                ...
                else if (compressionMethod == 1)
                {
                    ...
                    for (var i = 0; i < blockCount; i++)
                    {
                        ...
                        var input = new byte[compressedBlockLength];
                        var output = new byte[uncompressedBlockLengthArray[i]];

                        RawBinaryReader.Read(input, 0, compressedBlockLength);
                        lz4decoder.DecodeAndDrain(input, output, out _);

                        uncompressedBlocks.Write(output, 0, output.Length);
                    }
                }
                else if (compressionMethod == 2)
                {
