...
namespace ValveResourceFormat.ResourceTypes
{
    public class Material : KeyValuesOrNTRO
    {
        ...
        public byte[] ToValveMaterial()
        {
            ...
            {
                root.Add(new KVObject(i.Key, i.Value));
            }

            foreach (var i in FloatParams)
            {
                root.Add(new KVObject(i.Key, i.Value));
            }

            foreach (var i in VectorParams)
            {
                var value = i.Value;
                root.Add(new KVObject(i.Key, $"[{value.X} {value.Y} {value.Z} {value.W}]"));
            }

            foreach (var i in TextureParams)
            {
                root.Add(new KVObject(i.Key, i.Value));
            }

            foreach (var i in IntAttributes)
            {
                root.Add(new KVObject(i.Key, i.Value));
            }

            foreach (var i in FloatAttributes)
            {
                root.Add(new KVObject(i.Key, i.Value));
            }
            ...
            foreach (var i in VectorAttributes)
            {
                var value = i.Value;
                root.Add(new KVObject(i.Key, $"[{value.X} {value.Y} {value.Z} {value.W}]"));
            }

            foreach (var i in StringAttributes)
            {
                root.Add(new KVObject(i.Key, i.Value ?? string.Empty));
            }
